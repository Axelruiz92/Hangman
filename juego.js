let palabrita  ;
let cant_errores = 0; //cuantas veces me equivoque
let cant_aciertos = 0; //cuantas veces acerte

const palabras = [
    'Inception',        /* Indice 0 */ 
    'Titanic',          /* 1 */
    'Interestellar',    /* 2 */
    'Juno',             /* 3 */
    'Carrie',           /* 4 */
    'Shrek',            /* 5 */
    'Alien',            /* 6 */
    'Depredador',       /* 7 */
    'Madagascar',       /* 8 */
    'Spiderman',        /* 9 */
    'Vengadores',          /* 10 */
    'Cenicienta',
    'Terminator',
    'Matrix',
    'Scream',
    'Naufrago',
    'Hannibal',
    'Psicosis',
    'Supercool',
    'Zombieland',
    'Whiplash',
    'Thor',
     
]

const btn = id('jugar');
const imagen = id('imagen');
const btn_letras = document.querySelectorAll('#letras button');

//click en iniciar juego
btn.addEventListener('click', iniciar);


function iniciar(event) {
    id('resultado').innerHTML = '';
    imagen.src = 'imagenes/img0.png';
    btn.disabled = true;
    cant_errores = 0; //cuantas veces me equivoque
    cant_aciertos = 0; //cuantas veces acerte

    const parrafo = id('palabra_a_adivinar');
    parrafo.innerHTML = '';

    const cant_palabras = palabras.length;
    const valor_al_azar = obtener_random (0, cant_palabras);

    palabrita = palabras [valor_al_azar];
    console.log(palabrita);
    const cant_letras = palabrita.length; 

    for (let i=0; i< btn_letras.length; i++) {
        btn_letras [i].disabled = false;
    }

    for ( let i=0; i < cant_letras; i++ ) {
        const span = document.createElement('span');
        parrafo.appendChild( span );
    }


}

//click de adivinar letras
for (let i=0; i< btn_letras.length; i++) {
    btn_letras [i].addEventListener ('click', click_letras);
}

function click_letras (event) {
    const spans = document.querySelectorAll('#palabra_a_adivinar span');
    const button = event.target;
    button.disabled = true;
    const letra = button.innerHTML.toUpperCase();
    const palabra = palabrita.toUpperCase(); //toUpperCase;

    let acerto = false;
    for (let i=0; i <palabra.length ; i++) {
        if (letra == palabra[i] ) {
            //la variable i es la posicion de la letra en la palabra 
            //que coincide con el span al que tenemos que mostrarle esta letra.
            spans[i].innerHTML = letra;
            cant_aciertos++;
            acerto = true;
        }
    }

    if (acerto == false ) {
        cant_errores++;
            const source = `imagenes/img${cant_errores}.png`;
            const imagen = id('imagen');
            imagen.src = source;
    }

    if (cant_errores == 7) {
        id ('resultado').innerHTML = "Perdiste crack, la palabra era... " + palabrita + "!";
        game_over();        
    } else if (cant_aciertos == palabrita.length ) {
        id ('resultado').innerHTML = "Ganaste crack, honores a vos!"
        game_over();
    }

    console.log("la letra" + letra + "en la palabra" + palabra + "existe?:" + acerto);
}


//fin del juego
function game_over (){
    for (let i=0; i< btn_letras.length; i++) {
        btn_letras [i].disabled = true;
    }

    btn.disabled = false;

}

game_over ();